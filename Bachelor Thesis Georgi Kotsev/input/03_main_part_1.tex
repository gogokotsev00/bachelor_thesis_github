\chapter{Approach for Solving the Issue}
\label{mainone}
...
\begin{figure} [h!]
  ...
  \hfill
  \subfloat[Occlusion from side point of view]{\includegraphics[width=0.426\textwidth]{images/different_view_occlusion.jpg}}
...
In addition to the occlusion degree, we also rely on two metrics, when comparing the performance of cameras placed on different locations. The first one is how many times a vehicle on a specific position is not detected by a camera because the occlusion percentage exceeds the particular threshold for object recognition. Our second metric is the ratio of vehicles missed by the sensor in case they are out of its field of view to the total amount of traffic. When these two metrics have lower values, we are able to determine where is a reasonable area for camera installation, which will ensure a better traffic surveillance. 
